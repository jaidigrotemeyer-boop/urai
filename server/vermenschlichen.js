...
const ABKUERZUNG =
  /(?:\b(?:z\s?\.\s?B|bzw|u\s?\.\s?a|d\s?\.\s?h|ca|Dr|Prof|Nr|Abb|Bd|Std|usw|etc|vgl|ggf|inkl|max|min|Mrd|Mio|Jh|Hrsg|e\s?\.\s?g|i\s?\.\s?e|Mr|Mrs|Ms|St|Fig|No|vs)\.|\b\p{Lu}\.|\d\.)\s*$/u
...
function auffaelligkeiten(f) {
  ...
  if (f.absatz.anzahl >= 3 && f.absatz.gleichmass < 0.25)
    liste.push(`Absätze fast gleich lang (∅ ${f.absatz.mittel} Wörter, Streuung ${f.absatz.streuung}).`)
  return liste
}
